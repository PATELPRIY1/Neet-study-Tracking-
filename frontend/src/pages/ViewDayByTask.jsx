...
const ViewDayByTask = () => {
  ...
  const filteredTasks =
    filter === "all" ? task : task.filter((t) => t.done === filter);

  const completedCount = task.filter((t) => t.done === "completed").length;
  const pendingCount = task.filter((t) => t.done === "pending").length;
  ...
  async function handleDelete(taskId) {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`${VITE_API_URL}/api/deletetask/${taskId}`);
  ...
  async function handleDeleteAll() {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`${VITE_API_URL}/api/deletealltasks`);
        setTask([]);
            ...
            <div className="mb-4 flex items-center justify-between gap-4">
              <div className="mb-4 flex gap-4">
                {["all", "completed", "pending"].map((f) => (
                  ...
                  >
                    {f} ({f === "all" ? task.length : f === "completed" ? completedCount : pendingCount})
                  </button>
                ))}
              </div>
              ...
              <table className="min-w-full divide-y divide-white/10 text-sm">
                <thead className="bg-(--bg-transparent-2-color) text-left text-xs uppercase tracking-widest">
                  <tr>
                    ...
                    <th className="px-4 py-3">Biology</th>
                ...
                <tbody className="overflow-y-auto divide-y divide-white/10 text-(--secondary-color)">
                  {displayedTasks.length > 0 ? (
                    displayedTasks.map((taskItem) => {
                      const isCompleted = taskItem.done === "completed";
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-3">{taskItem.day || "-"}</td>
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`${statusClass} inline-flex px-3 py-1 text-xs font-semibold uppercase tracking-wide rounded-2xl`}
                            >
                              {statusLabel}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <button
                              onClick={() =>
                                handleUpdateStatus(taskItem._id, taskItem.done)
                              }
                              className="text-sm text-blue-500 hover:underline py-1 px-3 cursor-pointer"
                            >
                              Change Status
                            </button>
                          </td>
                          <td className="px-4 py-3">
                            <Link
                              to={`/update-day-task/${taskItem._id}`}
                    ...
                    <tr className="text-center text-gray-300">
                      <td colSpan="7" className="px-4 py-6">
                        No data available. Create a day task first.
                      </td>
                    </tr>
                  )}
                </tbody>
